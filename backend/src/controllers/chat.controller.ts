...
export const broadcastMessage = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ message: "Only premium sellers can use broadcast." });
    }
    ...
    if (sellerState) {
      ...
      stateUserIds = usersInState.map((u: any) => u.id);
    }

    // Combine unique user IDs
    const targetUserIds = Array.from(
      new Set([...chatUserIds, ...stateUserIds]),
    );
    if (targetUserIds.length === 0) {
      return res
        .status(200)
        .json({
          message: "No users found in chat history or same state.",
          sentCount: 0,
        });
    }
    ...
    if (listingId) {
      ...
      if (!listing || listing.sellerId !== userId) {
        return res
          .status(403)
          .json({ message: "Invalid listing or unauthorized." });
      }
    }
