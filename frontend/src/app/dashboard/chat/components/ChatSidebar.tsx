              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center gap-2 overflow-hidden">
                      {otherParticipant.profilePicture ? (
                        <img
                          ...
                          className={`w-8 h-8 rounded-full object-cover shrink-0 ${chat.isActive === false ? "grayscale opacity-50" : ""}`}
                        />
                      ) : (
                        <div
                          className={`w-8 h-8 rounded-full bg-primary/20 text-primary flex items-center justify-center font-bold text-sm shrink-0 ${chat.isActive === false ? "grayscale opacity-50" : ""}`}
                        >
                          {otherParticipant.name?.charAt(0).toUpperCase()}
                        </div>
                      )}
                      <h3
                        className={`font-bold text-sm truncate ${isActive ? "text-primary" : "text-foreground"} ${chat.isActive === false ? "opacity-50" : ""}`}
                      >
                        {otherParticipant.name}
                        {chat.isActive === false && (
                          <span className="ml-2 text-xs font-normal text-red-500 bg-red-50 dark:bg-red-900/30 dark:text-red-400 px-1 py-0.5 rounded">
                            Closed
                          </span>
                        )}
                      </h3>
                  ...
                  <div className="flex items-center justify-between">
                    <p
                      className={`text-sm truncate pr-2 ${isActive ? "text-primary font-medium" : "text-muted-foreground"}`}
                    >
                      {lastMsgContent}
                    </p>
