...
export default function BroadcastModal({ isOpen, onClose }: BroadcastModalProps) {
    ...
    useEffect(() => {
        if (isOpen && token) {
            api.get('/marketplace/my-listings', token).then(data => {
                if (Array.isArray(data)) {
                    setListings(data.filter(l => l.status === 'ACTIVE' || l.active));
                }
            }).catch(err => console.error("Failed to load listings for broadcast", err));
        }
    }, [isOpen, token]);
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                    <p className="text-sm text-gray-500">
                        Send a message to everyone you've chatted with previously, as well as all users in your state.
                    </p>
                    <textarea
                        value={content}
                    ...
                    {listings.length > 0 && (
                        <div className="space-y-2">
                            ...
                            >
                                ...
                                {listings.map(l => (
                                    <option key={l.id} value={l.id}>{l.title} (${Number(l.price).toFixed(2)})</option>
                                ))}
                            </select>
                        </div>
                    )}
                    ...
                    {resultMsg && (
                        <div className={`p-2 rounded text-sm ${resultMsg.startsWith('Success') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                            {resultMsg}
                        </div>
                    )}
                </div>
