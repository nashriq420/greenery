        ...
        <div className="p-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-muted-foreground">
                by{" "}
                <Link
                  href={`/dashboard/seller/${listing.sellerId}`}
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {listing.deliveryAvailable && (
              <span className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 text-sm px-3 py-1 rounded-full font-semibold border border-green-200 dark:border-green-900/50">
                Delivery Available
              </span>
            )}
            {listing.minQuantity && listing.minQuantity > 1 && (
              <span className="bg-muted text-muted-foreground text-sm px-3 py-1 rounded-full font-semibold border border-border">
                Min Qty: {listing.minQuantity}
              </span>
            )}
            {listing.strainType && (
              <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400 text-sm px-3 py-1 rounded-full font-semibold border border-purple-200 dark:border-purple-900/50">
                {listing.strainType}
              </span>
            )}
          ...
          <div className="mt-4 text-sm text-foreground bg-muted/30 p-4 rounded-lg border border-border space-y-2">
            {listing.type && (
              <div className="flex">
                <span className="font-semibold w-24 text-foreground">
                  Type:
                </span>
          ...
          <p className="mt-6 text-foreground whitespace-pre-line">
            {listing.description}
          </p>
          <div className="mt-6 text-sm text-muted-foreground">
            {listing.seller?.sellerProfile?.city},{" "}
            {listing.seller?.sellerProfile?.state}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {listing.reviews.length === 0 ? (
            ...
            listing.reviews.map((review) => (
              <div
                key={review.id}
                className="bg-card text-card-foreground p-6 rounded-lg shadow border border-border"
              >
