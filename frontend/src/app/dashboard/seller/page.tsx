...
export default function SellersPage() {
    const [location, setLocation] = useState<{ lat: number; lng: number }>({ lat: 51.505, lng: -0.09 }); // Default to London
    ...
    return (
        <div className="space-y-6">
            ...
            {loading ? (
                ...
                <div className="text-center py-10 text-gray-500">No sellers found nearby.</div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {sellers.map((seller) => (
                        <Card key={seller.id} className="hover:shadow-lg transition-shadow">
                            ...
                            <CardContent>
                                <p className="text-sm text-gray-600 mb-2">{seller.city || 'Location unknown'}</p>
