...
const TAG_COLORS: Record<string, { bg: string; text: string; border: string }> = {
    'general': { bg: 'bg-slate-100 dark:bg-slate-800', text: 'text-slate-700 dark:text-slate-300', border: 'border-slate-300 dark:border-slate-600' },
    'marketplace': { bg: 'bg-blue-100 dark:bg-blue-900/40', text: 'text-blue-700 dark:text-blue-300', border: 'border-blue-300 dark:border-blue-600' },
    'listing': { bg: 'bg-purple-100 dark:bg-purple-900/40', text: 'text-purple-700 dark:text-purple-300', border: 'border-purple-300 dark:border-purple-600' },
    'vendor': { bg: 'bg-orange-100 dark:bg-orange-900/40', text: 'text-orange-700 dark:text-orange-300', border: 'border-orange-300 dark:border-orange-600' },
    'growing-tips': { bg: 'bg-green-100 dark:bg-green-900/40', text: 'text-green-700 dark:text-green-300', border: 'border-green-300 dark:border-green-600' },
...
export default function CommunitySidebar({ selectedTag, onTagSelect, postCount = 0 }: CommunitySidebarProps) {
    ...
    const allTags = data?.allTags ?? [
        ...
        { tag: 'marketplace', label: '🛒 Marketplace', recentCount: 0, totalCount: 0 },
        { tag: 'listing', label: '📋 Listing', recentCount: 0, totalCount: 0 },
        { tag: 'vendor', label: '🏪 Vendor', recentCount: 0, totalCount: 0 },
        { tag: 'growing-tips', label: '🌱 Growing Tips', recentCount: 0, totalCount: 0 },
        ...
        <div className="space-y-4">
            ...
            <div className="bg-card border border-border rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                    <div className="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-full flex items-center justify-center text-lg">🌿</div>
                    <div>
                        <h2 className="font-bold text-sm text-foreground">r/Greenery</h2>
                        <p className="text-xs text-muted-foreground">The cannabis community</p>
                    </div>
                </div>
            ...
            <div className="bg-card border border-border rounded-xl p-4 shadow-sm">
                ...
                {loading ? (
                    ...
                    <p className="text-xs text-muted-foreground text-center py-2">No trending topics yet. Be the first to post!</p>
                ) : (
