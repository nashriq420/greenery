        ...
        <div className="bg-card border border-border text-card-foreground rounded-2xl overflow-hidden shadow-sm hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 relative group">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="text-xs text-muted-foreground flex items-center flex-wrap gap-1.5 mt-0.5 font-medium">
                            ...
                            {(post.isEdited || currentContent !== post.content) && <span className="text-[10px] italic opacity-70">(edited)</span>}
                            {post.author.role === 'SELLER' && (
                                <>
                                    <span className="opacity-50">•</span>
                                    {post.author.subscription?.status === 'ACTIVE' ? (
                                        <span className="px-1.5 py-0.5 bg-yellow-400/10 text-yellow-600 dark:text-yellow-500 text-[9px] rounded-md font-bold uppercase tracking-wider border border-yellow-400/20">Premium</span>
                                    ) : (
                                        <span className="px-1.5 py-0.5 bg-primary/10 text-primary text-[9px] rounded-md font-bold uppercase tracking-wider border border-primary/20">Seller</span>
                                    )}
                                </>
                            )}
                        </div>
                ...
                <div className="flex items-center gap-3">
                    {post.tag && post.tag !== 'general' && (
                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold border uppercase tracking-wider ${
                            post.tag === 'marketplace' ? 'bg-blue-100/50 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800/50' :
                            post.tag === 'listing' ? 'bg-purple-100/50 text-purple-700 border-purple-200 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800/50' :
                            post.tag === 'vendor' ? 'bg-orange-100/50 text-orange-700 border-orange-200 dark:bg-orange-900/30 dark:text-orange-400 dark:border-orange-800/50' :
                            post.tag === 'growing-tips' ? 'bg-green-100/50 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800/50' :
                            post.tag === 'questions' ? 'bg-yellow-100/50 text-yellow-700 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-400 dark:border-yellow-800/50' :
            ...
            <div className={`px-5 pb-3 ${post.imageUrl ? 'mb-2' : ''}`}>
                {isEditing ? (
                    ...
                    <p className="text-foreground text-[15px] leading-relaxed whitespace-pre-wrap">{currentContent}</p>
                )}
            </div>
            ...
            <div className="px-5 py-3 border-t border-border bg-muted/10 flex items-center gap-2">
                ...
                >
                    <Heart size={18} className={`${isLiked ? 'fill-current' : ''} ${isAnimating ? 'animate-bounce' : ''}`} />
                                            ...
                                            <div className="flex items-center gap-1.5 font-bold text-xs text-foreground mb-1">
                                                ...
                                                {comment.author.role === 'SELLER' && (
                                                    comment.author.subscription?.status === 'ACTIVE' ? (
                                                        <span className="px-1.5 py-0.5 bg-yellow-400/10 text-yellow-600 dark:text-yellow-500 text-[8px] rounded-md uppercase tracking-wider ml-1 border border-yellow-400/20">Premium</span>
                                                    ) : (
                                                        <span className="px-1.5 py-0.5 bg-primary/10 text-primary text-[8px] rounded-md uppercase tracking-wider ml-1 border border-primary/20">Seller</span>
                                                    )
                                                )}
                                            </div>
                            ...
                            <div className="relative pt-2">
                                ...
                                <form onSubmit={handleAddComment} className="flex gap-3 items-start">
                                    <div className="hidden sm:flex w-8 h-8 bg-primary/10 rounded-full items-center justify-center text-primary font-bold overflow-hidden border border-primary/20 shrink-0 mt-0.5">
                                        {user?.profilePicture ? (
                                            <Image src={user.profilePicture} alt="You" fill className="object-cover" />
                                        ) : (
