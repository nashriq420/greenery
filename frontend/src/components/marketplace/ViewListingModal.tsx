                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                            {listing.sku && (
                                <span className="bg-muted text-muted-foreground text-xs px-2 py-1 rounded-full border border-border">SKU: {listing.sku}</span>
                            )}
                            {listing.deliveryAvailable && (
                                <span className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 text-xs px-2 py-1 rounded-full font-semibold">Delivery Available</span>
                            )}
                            {listing.minQuantity && listing.minQuantity > 1 && (
                                <span className="bg-muted text-foreground text-xs px-2 py-1 rounded-full font-semibold border border-border">Min Qty: {listing.minQuantity}</span>
                            )}
                            {listing.type && (
                            ...
                            <div className="mt-4 space-y-2">
                                {listing.flavors && (
                                    <div className="flex items-start gap-2">
                                        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider min-w-[60px] pt-1">Flavours:</span>
                                        <div className="flex flex-wrap gap-1">
                                            {listing.flavors.split(',').map((flavor, i) => (
                                                <span key={i} className="text-xs bg-orange-50 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 px-2 py-0.5 rounded border border-orange-100 dark:border-orange-900/50">
                                                    {flavor.trim()}
                                                </span>
                                            ))}
                                        </div>
                                ...
                                {listing.effects && (
                                    <div className="flex items-start gap-2">
                                        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider min-w-[60px] pt-1">Effects:</span>
                                        <div className="flex flex-wrap gap-1">
                                            {listing.effects.split(',').map((effect, i) => (
                                                <span key={i} className="text-xs bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 px-2 py-0.5 rounded border border-teal-100 dark:border-teal-900/50">
                                                    {effect.trim()}
                                                </span>
                                            ))}
                                        </div>
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1 text-sm whitespace-pre-wrap">{listing.description}</p>
                    </div>
